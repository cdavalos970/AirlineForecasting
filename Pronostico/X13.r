...
X13_forecast_USD_Real <- data.frame()
X13_forecast_COP_Real <- data.frame()
...
cantDatos <- nrow(Pax_Real[posLeg])
	...
	X13_Pax_Real <- seas(ts_Pax_Real)
	matriz_Pax_Real <- rbind(matriz_Pax_Real,as.data.frame(series(X13_Pax_Real,"fct"))[periods,1])
	...
	X13_Pax_Outlier <- seas(ts_Pax_Outlier)
	matriz_Pax_Outlier <- rbind(matriz_Pax_Outlier,as.data.frame(series(X13_Pax_Outlier,"fct"))[periods,1])
	...
	X13_Rev_USD_Real <- seas(ts_Rev_USD_Real)
	matriz_Rev_USD_Real <- rbind(matriz_Rev_USD_Real,as.data.frame(series(X13_Rev_USD_Real,"fct"))[periods,1])
	...
	X13_Rev_USD_Outlier <- seas(ts_Rev_USD_Outlier)
	matriz_Rev_USD_Outlier <- rbind(matriz_Rev_USD_Outlier,as.data.frame(series(X13_Rev_USD_Outlier,"fct"))[periods,1])
	...
	X13_Rev_COP_Real <- seas(ts_Rev_COP_Real)
	matriz_Rev_COP_Real <- rbind(matriz_Rev_COP_Real,as.data.frame(series(X13_Rev_COP_Real, "fct"))[periods,1])
	} else {
	...
	X13_Rev_COP_Outlier <- seas(ts_Rev_COP_Outlier)
	matriz_Rev_COP_Outlier <- rbind(matriz_Rev_COP_Outlier,as.data.frame(series(X13_Rev_COP_Outlier,"fct"))[periods,1])
...
X13_forecast_Pax_Real <- t(matriz_Pax_Real)
X13_forecast_USD_Real <- t(matriz_Rev_USD_Real)
X13_forecast_COP_Real <- t(matriz_Rev_COP_Real)

X13_forecast_Pax_Outlier <- t(matriz_Pax_Outlier)
X13_forecast_Rev_USD_Outlier <- t(matriz_Rev_USD_Outlier)
X13_forecast_Rev_COP_Outlier <- t(matriz_Rev_COP_Outlier)

colnames(X13_forecast_Pax_Real) <- as.character(Pax_Real_Leg)
colnames(X13_forecast_USD_Real) <- as.character(REV_USD_Real_Leg)
colnames(X13_forecast_COP_Real) <- as.character(Rev_COP_Real_Leg)

colnames(X13_forecast_Pax_Outlier) <- as.character(Pax_Outlier_Leg)
colnames(X13_forecast_Rev_USD_Outlier) <- as.character(REV_USD_Outliers_Leg)
colnames(X13_forecast_Rev_COP_Outlier) <- as.character(Rev_COP_Outliers_Leg)

rownames(X13_forecast_Pax_Real) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)
rownames(X13_forecast_USD_Real) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)
rownames(X13_forecast_COP_Real) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)
...
write.table(file="PAX_Real.csv",X13_forecast_Pax_Real)
write.table(file="REV_USD_Real.csv",X13_forecast_USD_Real)
write.table(file="REV_COP_Real.csv",X13_forecast_COP_Real)

write.table(file="PAX_Outliers.csv",X13_forecast_Pax_Outlier)
write.table(file="REV_USD_Outliers.csv",X13_forecast_Rev_USD_Outlier)
write.table(file="REV_COP_Outliers.csv",X13_forecast_Rev_COP_Outlier)