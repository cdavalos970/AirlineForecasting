...
matriz_Pax_Real_ETS <- data.frame()
...
ETS_forecast_Pax_Real <- data.frame()
ETS_forecast_USD_Real <- data.frame()
ETS_forecast_COP_Real <- data.frame()

ETS_forecast_Pax_Outlier <- data.frame()
ETS_forecast_Rev_USD_Outlier <- data.frame()
ETS_forecast_Rev_COP_Outlier <- data.frame()
...
cantDatos <- nrow(Pax_Real[posLeg])
	...
	ETS_Pax_Real <- ets(ts_Pax_Real)
	matriz_Pax_Real <- rbind(matriz_Pax_Real,as.data.frame(forecast(ETS_Pax_Real))[periods,1])
	...
	ETS_Pax_Outlier <- ets(ts_Pax_Outlier)
	matriz_Pax_Outlier <- rbind(matriz_Pax_Outlier,as.data.frame(forecast(ETS_Pax_Outlier))[periods,1])
	...
	ETS_Rev_USD_Real <- ets(ts_Rev_USD_Real)
	matriz_Rev_USD_Real <- rbind(matriz_Rev_USD_Real,as.data.frame(forecast(ETS_Rev_USD_Real))[periods,1])
	...
	ETS_Rev_USD_Outlier <- ets(ts_Rev_USD_Outlier)
	matriz_Rev_USD_Outlier <- rbind(matriz_Rev_USD_Outlier,as.data.frame(forecast(ETS_Rev_USD_Outlier))[periods,1])
	...
	ETS_Rev_COP_Real <- ets(ts_Rev_COP_Real)
	matriz_Rev_COP_Real <- rbind(matriz_Rev_COP_Real,as.data.frame(forecast(ETS_Rev_COP_Real))[periods,1])
	} else {
	...
	ETS_Rev_COP_Outlier <- ets(ts_Rev_COP_Outlier)
	matriz_Rev_COP_Outlier <- rbind(matriz_Rev_COP_Outlier,as.data.frame(forecast(ETS_Rev_COP_Outlier))[periods,1])
	}
	}
}


ETS_forecast_Pax_Real <- t(matriz_Pax_Real)
ETS_forecast_USD_Real <- t(matriz_Rev_USD_Real)
ETS_forecast_COP_Real <- t(matriz_Rev_COP_Real)

ETS_forecast_Pax_Outlier <- t(matriz_Pax_Outlier)
ETS_forecast_Rev_USD_Outlier <- t(matriz_Rev_USD_Outlier)
ETS_forecast_Rev_COP_Outlier <- t(matriz_Rev_COP_Outlier)

colnames(ETS_forecast_Pax_Real) <- as.character(Pax_Real_Leg)
colnames(ETS_forecast_USD_Real) <- as.character(REV_USD_Real_Leg)
colnames(ETS_forecast_COP_Real) <- as.character(Rev_COP_Real_Leg)

colnames(ETS_forecast_Pax_Outlier) <- as.character(Pax_Outlier_Leg)
colnames(ETS_forecast_Rev_USD_Outlier) <- as.character(REV_USD_Outliers_Leg)
colnames(ETS_forecast_Rev_COP_Outlier) <- as.character(Rev_COP_Outliers_Leg)
...
write.table(file="PAX_Real.csv",ETS_forecast_Pax_Real)
write.table(file="REV_USD_Real.csv",ETS_forecast_USD_Real)
write.table(file="REV_COP_Real.csv",ETS_forecast_COP_Real)

write.table(file="PAX_Outliers.csv",ETS_forecast_Pax_Outlier)
write.table(file="REV_USD_Outliers.csv",ETS_forecast_Rev_USD_Outlier)
write.table(file="REV_COP_Outliers.csv",ETS_forecast_Rev_COP_Outlier)