...
cantDatos <- nrow(Pax_Real[posLeg])
	...
	matriz_Pax_Real <- rbind(matriz_Pax_Real,as.data.frame(forecast(Arima_Pax_Real))[periods,1])
	...
	matriz_Pax_Outlier <- rbind(matriz_Pax_Outlier,as.data.frame(forecast(Arima_Pax_Outlier))[periods,1])
	...
	matriz_Rev_USD_Real <- rbind(matriz_Rev_USD_Real,as.data.frame(forecast(Arima_Rev_USD_Real))[periods,1])
	...
	matriz_Rev_USD_Outlier <- rbind(matriz_Rev_USD_Outlier,as.data.frame(forecast(Arima_Rev_USD_Outlier))[periods,1])
	...
	matriz_Rev_COP_Real <- rbind(matriz_Rev_COP_Real,as.data.frame(forecast(Arima_Rev_COP_Real))[periods,1])
	} else {
	...
	matriz_Rev_COP_Outlier <- rbind(matriz_Rev_COP_Outlier,as.data.frame(forecast(Arima_Rev_COP_Outlier))[periods,1])
...
colnames(Arima_forecast_Pax_Real) <- as.character(Pax_Real_Leg)
colnames(Arima_forecast_USD_Real) <- as.character(REV_USD_Real_Leg)
colnames(Arima_forecast_COP_Real) <- as.character(Rev_COP_Real_Leg)

colnames(Arima_forecast_Pax_Outlier) <- as.character(Pax_Outlier_Leg)
colnames(Arima_forecast_Rev_USD_Outlier) <- as.character(REV_USD_Outliers_Leg)
colnames(Arima_forecast_Rev_COP_Outlier) <- as.character(Rev_COP_Outliers_Leg)
...
write.table(file="PAX_Real.csv",Arima_forecast_Pax_Real)
write.table(file="REV_USD_Real.csv",Arima_forecast_USD_Real)
write.table(file="REV_COP_Real.csv",Arima_forecast_COP_Real)

write.table(file="PAX_Outliers.csv",Arima_forecast_Pax_Outlier)
write.table(file="REV_USD_Outliers.csv",Arima_forecast_Rev_USD_Outlier)
write.table(file="REV_COP_Outliers.csv",Arima_forecast_Rev_COP_Outlier)