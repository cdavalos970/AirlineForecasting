library(forecast)
library(seasonal)
setwd("C:/Users/cdavalos/Documents/Pronostico TM/R/Pronosticos/Damped Trend")
...
holt_forecast_Pax_Real <- data.frame()
holt_forecast_USD_Real <- data.frame()
holt_forecast_COP_Real <- data.frame()

holt_forecast_Pax_Outlier <- data.frame()
holt_forecast_Rev_USD_Outlier <- data.frame()
holt_forecast_Rev_COP_Outlier <- data.frame()
...
cantDatos <- nrow(Pax_Real[posLeg])
	...
	holt_Pax_Real <- holt(ts_Pax_Real,exponential=TRUE,damped=TRUE)
	matriz_Pax_Real <- rbind(matriz_Pax_Real,as.data.frame(holt_Pax_Real)[c(1,2,3),1])
	...
	holt_Pax_Outlier <- holt(ts_Pax_Outlier,exponential=TRUE,damped=TRUE)
	matriz_Pax_Outlier <- rbind(matriz_Pax_Outlier,as.data.frame(holt_Pax_Outlier)[c(1,2,3),1])
	...
	holt_Rev_USD_Real <- holt(ts_Rev_USD_Real,exponential=TRUE,damped=TRUE)
	matriz_Rev_USD_Real <- rbind(matriz_Rev_USD_Real,as.data.frame(holt_Rev_USD_Real)[c(1,2,3),1])
	...
	holt_Rev_USD_Outlier <- holt(ts_Rev_USD_Outlier,exponential=TRUE,damped=TRUE)
	matriz_Rev_USD_Outlier <- rbind(matriz_Rev_USD_Outlier,as.data.frame(holt_Rev_USD_Outlier)[c(1,2,3),1])
	...
	holt_Rev_COP_Real <- holt(ts_Rev_COP_Real,exponential=TRUE,damped=TRUE)
	matriz_Rev_COP_Real <- rbind(matriz_Rev_COP_Real,as.data.frame(holt_Rev_COP_Real)[c(1,2,3),1])
	} else {
	...
	holt_Rev_COP_Outlier <- holt(ts_Rev_COP_Outlier,exponential=TRUE,damped=TRUE)
	matriz_Rev_COP_Outlier <- rbind(matriz_Rev_COP_Outlier,as.data.frame(holt_Rev_COP_Outlier)[c(1,2,3),1])
...
holt_forecast_Pax_Real <- t(matriz_Pax_Real)
holt_forecast_USD_Real <- t(matriz_Rev_USD_Real)
holt_forecast_COP_Real <- t(matriz_Rev_COP_Real)

holt_forecast_Pax_Outlier <- t(matriz_Pax_Outlier)
holt_forecast_Rev_USD_Outlier <- t(matriz_Rev_USD_Outlier)
holt_forecast_Rev_COP_Outlier <- t(matriz_Rev_COP_Outlier)

colnames(holt_forecast_Pax_Real) <- as.character(Pax_Real_Leg)
colnames(holt_forecast_USD_Real) <- as.character(REV_USD_Real_Leg)
colnames(holt_forecast_COP_Real) <- as.character(Rev_COP_Real_Leg)

colnames(holt_forecast_Pax_Outlier) <- as.character(Pax_Outlier_Leg)
colnames(holt_forecast_Rev_USD_Outlier) <- as.character(REV_USD_Outliers_Leg)
colnames(holt_forecast_Rev_COP_Outlier) <- as.character(Rev_COP_Outliers_Leg)

rownames(holt_forecast_Pax_Real) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)
rownames(holt_forecast_USD_Real) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)
rownames(holt_forecast_COP_Real) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)

rownames(holt_forecast_Pax_Outlier) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)
rownames(holt_forecast_Rev_USD_Outlier) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)
rownames(holt_forecast_Rev_COP_Outlier) <- seq(as.Date("2015/1/1"),by = "month", length.out = forecast_periods)

write.table(file="PAX_Real.csv",holt_forecast_Pax_Real)
write.table(file="REV_USD_Real.csv",holt_forecast_USD_Real)
write.table(file="REV_COP_Real.csv",holt_forecast_COP_Real)

write.table(file="PAX_Outliers.csv",holt_forecast_Pax_Outlier)
write.table(file="REV_USD_Outliers.csv",holt_forecast_Rev_USD_Outlier)
write.table(file="REV_COP_Outliers.csv",holt_forecast_Rev_COP_Outlier)