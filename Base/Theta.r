...
theta_forecast_Error_Pax_Real <- data.frame()
theta_forecast_Error_USD_Real <- data.frame()
theta_forecast_Error_COP_Real <- data.frame()

theta_forecast_Error_Pax_Outlier <- data.frame()
theta_forecast_Error_Rev_USD_Outlier <- data.frame()
theta_forecast_Error_Rev_COP_Outlier <- data.frame()
...
#i=3
   ...
   fit <- thetaf(train,h = h,level=c(confidence_level), fan=FALSE)                                                                                                            #Corre ARIMA
   fc <- forecast(fit,h = h) 
...
theta_Pax_Real <- thetaf(ts_Pax_Real,h = forecast_periods,level=c(confidence_level), fan=FALSE)
Indicadores_Pax_Real <- getrmse(ts_Pax_Real, forecast_periods, iterations)
theta_Rev_USD_Real <- thetaf(ts_Rev_USD_Real,h = forecast_periods,level=c(confidence_level), fan=FALSE)
Indicadores_Rev_USD_Real <- getrmse(ts_Rev_USD_Real, forecast_periods, iterations)
theta_Rev_COP_Real <- thetaf(ts_Rev_COP_Real,h = forecast_periods,level=c(confidence_level), fan=FALSE)
Indicadores_Rev_COP_Real <- getrmse(ts_Rev_COP_Real, forecast_periods, iterations)

theta_Pax_Outlier <- thetaf(ts_Pax_Outlier,h = forecast_periods,level=c(confidence_level), fan=FALSE)
Indicadores_Pax_Outlier  <- getrmse(ts_Pax_Outlier, forecast_periods, iterations)
theta_Rev_USD_Outlier <- thetaf(ts_Rev_USD_Outlier,h = forecast_periods,level=c(confidence_level), fan=FALSE)
Indicadores_Rev_USD_Outlier  <- getrmse(ts_Rev_USD_Outlier, forecast_periods, iterations)
theta_Rev_COP_Outlier <- thetaf(ts_Rev_COP_Outlier,h = forecast_periods,level=c(confidence_level), fan=FALSE)
Indicadores_Rev_COP_Outlier  <- getrmse(ts_Rev_COP_Outlier, forecast_periods, iterations)

matriz_Pax_Real <- rbind(matriz_Pax_Real,as.data.frame(theta_Pax_Real)[c(1,2,3),1])
matriz_Rev_USD_Real <- rbind(matriz_Rev_USD_Real,as.data.frame(theta_Rev_USD_Real)[c(1,2,3),1])
matriz_Rev_COP_Real <- rbind(matriz_Rev_COP_Real,as.data.frame(theta_Rev_COP_Real)[c(1,2,3),1])

matriz_Pax_Outlier <- rbind(matriz_Pax_Outlier,as.data.frame(theta_Pax_Outlier)[c(1,2,3),1])
matriz_Rev_USD_Outlier <- rbind(matriz_Rev_USD_Outlier,as.data.frame(theta_Rev_USD_Outlier)[c(1,2,3),1])
matriz_Rev_COP_Outlier <- rbind(matriz_Rev_COP_Outlier,as.data.frame(theta_Rev_COP_Outlier)[c(1,2,3),1])
...
theta_forecast_Error_Pax_Real <- t(matriz_Indicadores_Pax_Real)
theta_forecast_Error_USD_Real <- t(matriz_Indicadores_Rev_USD_Real)
theta_forecast_Error_COP_Real <- t(matriz_Indicadores_Rev_COP_Real)

theta_forecast_Error_Pax_Outlier <- t(matriz_Indicadores_Pax_Outlier)
theta_forecast_Error_Rev_USD_Outlier <- t(matriz_Indicadores_Rev_USD_Outlier)
theta_forecast_Error_Rev_COP_Outlier <- t(matriz_Indicadores_Rev_COP_Outlier)

colnames(theta_forecast_Pax_Real) <- colnames(Pax_Real)
colnames(theta_forecast_USD_Real) <- colnames(Pax_Real)
colnames(theta_forecast_COP_Real) <- colnames(Pax_Real)

colnames(theta_forecast_Pax_Outlier) <- colnames(Pax_Real)
colnames(theta_forecast_Rev_USD_Outlier) <- colnames(Pax_Real)
colnames(theta_forecast_Rev_COP_Outlier) <- colnames(Pax_Real)

colnames(theta_forecast_Error_Pax_Real) <- colnames(Pax_Real)
colnames(theta_forecast_Error_USD_Real) <- colnames(Pax_Real)
colnames(theta_forecast_Error_COP_Real) <- colnames(Pax_Real)

colnames(theta_forecast_Error_Pax_Outlier) <- colnames(Pax_Real)
colnames(theta_forecast_Error_Rev_USD_Outlier) <- colnames(Pax_Real)
colnames(theta_forecast_Error_Rev_COP_Outlier) <- colnames(Pax_Real)
...
rownames(theta_forecast_Error_Pax_Real) <- Vector_Indicadores
rownames(theta_forecast_Error_USD_Real) <- Vector_Indicadores
rownames(theta_forecast_Error_COP_Real) <- Vector_Indicadores

rownames(theta_forecast_Error_Pax_Outlier) <- Vector_Indicadores
rownames(theta_forecast_Error_Rev_USD_Outlier) <- Vector_Indicadores
rownames(theta_forecast_Error_Rev_COP_Outlier) <- Vector_Indicadores

write.table(file="PAX_Outliers.csv",theta_forecast_Pax_Real)
write.table(file="REV_USD_Outliers.csv",theta_forecast_USD_Real)
write.table(file="REV_COP_Outliers.csv",theta_forecast_COP_Real)

write.table(file="PAX_Real.csv",theta_forecast_Pax_Outlier)
write.table(file="REV_USD_Real.csv",theta_forecast_Rev_USD_Outlier)
write.table(file="REV_COP_Real.csv",theta_forecast_Rev_COP_Outlier)

setwd("C:/Users/cdavalos/Documents/Pronostico TM/R/Indicadores/Theta")

write.table(file="Indicadores_PAX_Real.csv",theta_forecast_Error_Pax_Real)
write.table(file="Indicadores_REV_USD_Real.csv",theta_forecast_Error_USD_Real)
write.table(file="Indicadores_REV_COP_Real.csv",theta_forecast_Error_COP_Real)

write.table(file="Indicadores_PAX_Outliers.csv",theta_forecast_Error_Pax_Outlier)
write.table(file="Indicadores_REV_USD_Outliers.csv",theta_forecast_Error_Rev_USD_Outlier)
write.table(file="Indicadores_REV_COP_Outliers.csv",theta_forecast_Error_Rev_COP_Outlier)
