...
hw_forecast_Pax_Real <- data.frame()
hw_forecast_USD_Real <- data.frame()
hw_forecast_COP_Real <- data.frame()

hw_forecast_Pax_Outlier <- data.frame()
hw_forecast_Rev_USD_Outlier <- data.frame()
hw_forecast_Rev_COP_Outlier <- data.frame()
...
#i=3
   ...
   fit <- holt(train, exponential=TRUE,damped=TRUE)                                                                                                            #Corre ARIMA
   fc <- forecast(fit,h = h) 
...
hw_Pax_Real <- holt(ts_Pax_Real,exponential=TRUE,damped=TRUE)
Indicadores_Pax_Real <- getrmse(ts_Pax_Real, forecast_periods, iterations)
hw_Rev_USD_Real <- holt(ts_Rev_USD_Real,exponential=TRUE,damped=TRUE)
Indicadores_Rev_USD_Real <- getrmse(ts_Rev_USD_Real, forecast_periods, iterations)
hw_Rev_COP_Real <- holt(ts_Rev_COP_Real,exponential=TRUE,damped=TRUE)
Indicadores_Rev_COP_Real <- getrmse(ts_Rev_COP_Real, forecast_periods, iterations)

hw_Pax_Outlier <- holt(ts_Pax_Outlier,exponential=TRUE,damped=TRUE)
Indicadores_Pax_Outlier  <- getrmse(ts_Pax_Outlier, forecast_periods, iterations)
hw_Rev_USD_Outlier <- holt(ts_Rev_USD_Outlier,exponential=TRUE,damped=TRUE)
Indicadores_Rev_USD_Outlier  <- getrmse(ts_Rev_USD_Outlier, forecast_periods, iterations)
hw_Rev_COP_Outlier <- holt(ts_Rev_COP_Outlier,exponential=TRUE,damped=TRUE)
Indicadores_Rev_COP_Outlier  <- getrmse(ts_Rev_COP_Outlier, forecast_periods, iterations)

matriz_Pax_Real <- rbind(matriz_Pax_Real,as.data.frame(hw_Pax_Real)[c(1,2,3),1])
matriz_Rev_USD_Real <- rbind(matriz_Rev_USD_Real,as.data.frame(hw_Rev_USD_Real)[c(1,2,3),1])
matriz_Rev_COP_Real <- rbind(matriz_Rev_COP_Real,as.data.frame(hw_Rev_COP_Real)[c(1,2,3),1])

matriz_Pax_Outlier <- rbind(matriz_Pax_Outlier,as.data.frame(hw_Pax_Outlier)[c(1,2,3),1])
matriz_Rev_USD_Outlier <- rbind(matriz_Rev_USD_Outlier,as.data.frame(hw_Rev_USD_Outlier)[c(1,2,3),1])
matriz_Rev_COP_Outlier <- rbind(matriz_Rev_COP_Outlier,as.data.frame(hw_Rev_COP_Outlier)[c(1,2,3),1])
