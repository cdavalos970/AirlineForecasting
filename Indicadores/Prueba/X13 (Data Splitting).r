...
for(g in 1:legsPaxReal){
	...
	for(i in 1:j){
		...
		fc <- series(seas(train,outlier = NULL), "fct")[,1]
	...
	generarErrores <- cbind(as.data.frame(c(mean(me),mean(rmse), mean(mae), mean(mape))),l)
...
colnames(X13_forecast_Error_USD_Real) <- colnames(Rev_USD_Real)
colnames(X13_forecast_Error_COP_Real) <- colnames(Rev_COP_Real)

colnames(X13_forecast_Error_Pax_Outlier) <- colnames(Pax_Outliers)
colnames(X13_forecast_Error_Rev_USD_Outlier) <- colnames(Rev_USD_Outliers)
colnames(X13_forecast_Error_Rev_COP_Outlier) <- colnames(Rev_COP_Outliers)
...
setwd("C:/Users/cdavalos/Documents/Pronostico TM/R/Indicadores/Prueba/X13")
...
setwd("C:/Users/cdavalos/Documents/Pronostico TM/R/Prueba/Datos Data Splitting/X13")
